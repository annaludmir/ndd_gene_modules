...
def derive_outdir(summary_csv: str | Path) -> Path:
    ...
    return Path("results/OMIM") / f"{run_name}_OMIM_enrichment"
...
def run_omim_enrichment_per_condition(
    gene_lists: dict,
    outdir: str | Path,
    organism: str = "Human",
    ...
    for condition, genes in gene_lists.items():
        ...
        print(f"Running OMIM enrichment: {condition} (n_genes={len(genes)})")
        ...
        if not res.empty and "Adjusted P-value" in res.columns:
            ...
            if sig.empty:
                print(f"  No significant OMIM terms at cutoff={cutoff}")
        else:
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="Run OMIM disease enrichment (Enrichr ORA) per condition from a GSEA summary CSV."
    )
    parser.add_argument(
        "--summary-csv",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "If omitted, derived automatically as results/OMIM/{run_name}_OMIM_enrichment "
    ...
    parser.add_argument(
        ...
        metavar="GENE_SET",
        help=(
            "One or more Enrichr gene set libraries to query. "
            f"Defaults to: {DEFAULT_GENE_SETS}."
        ),
    )
    return parser
...
if __name__ == "__main__":
    ...
    run_omim_enrichment_per_condition(
        gene_lists,
        outdir=outdir,
